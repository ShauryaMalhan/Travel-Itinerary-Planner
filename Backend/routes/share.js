...
router.post('/', async (req, res) => {
  try {
    const { title, destination, days, totalDays } = req.body
    if (!destination || !Array.isArray(days) || !totalDays) {
      return res.status(400).json({ success: false, message: 'Invalid payload' })
    }
    const shortId = generateShortId()
    const doc = await SharedTrip.create({ shortId, title, destination, days, totalDays })
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { title: doc.title, destination: doc.destination, days: doc.days, totalDays: doc.totalDays, createdAt: doc.createdAt } })
  } catch (e) {
